...
def get_recs(ingredients, N=5, mean=False):
    ...
    # load in data
    data = pd.read_csv("parsed_ingredients.csv")
    # parse ingredients
    ...
    input_ = ingredient_parser(input)
    # get embeddings for ingredient doc
    if mean:
        input_embedding = mean_vec_tr.transform([input_])[0].reshape(1, -1)
    else:
        input_embedding = tfidf_vec_tr.transform([input_])[0].reshape(1, -1)
