...
def ingredient_parser(ingreds):
    ...
    words_to_remove = ['fresh', 'oil', 'a', 'red', 'bunch', 'and', 'clove', 'or', 'leaf', 'chilli', 'large', 'extra', 
                       'spring', 'ground', 'handful', 'free', 'small', 'pepper', 'virgin', 'range', 'from', 'dried', 
                       ...
                       'sliced', 'green', 'picked', 'the', 'stick', 'plain', 'plus', 'mixed', 'bay', 'your', 
                       'cumin', 'optional', 'fennel', 'serve', 'unsalted', 'baby', 'fat', 'ask', 
                       'natural', 'skin', 'roughly', 'into', 'such', 'cut', 'good', 'brown', 'grated', 'trimmed', 
                       'powder', 'yellow', 'dusting', 'knob', 'frozen', 'on', 'deseeded', 'low', 'runny', 'balsamic', 'cooked', 
                       'streaky', 'sage', 'rasher', 'zest', 'pin', 'groundnut', 'breadcrumb', 'halved', 
                       'grating', 'stalk', 'light', 'tinned', 'dry', 'soft', 'rocket', 'bone', 'colour', 'washed', 'skinless', 
                       'leftover', 'splash', 'removed', 'dijon', 'thick', 'big', 'hot', 'drained', 'sized',
                       'watercress', 'fishmonger', 'english', 'dill', 'caper', 'raw', 'worcestershire', 'flake', 'cider', 
                       'tbsp', 'leg', 'pine', 'wild', 'if', 'fine', 'herb', 'shoulder', 'cube', 'dressing', 
                       'with', 'chunk', 'spice', 'thumb', 'garam', 'new', 'little', 'punnet', 'peppercorn', 'shelled', 
                       'other','chopped', 'salt', 'taste', 'can', 'sauce', 'water', 'diced', 'package', 'italian', 
                       'shredded', 'divided', 'parsley', 'all', 'purpose', 'crushed', 'juice', 'more', 
                       'bell', 'needed', 'thinly', 'boneless', 'half', 'cubed', 'cinnamon', 'jar', 
                       'seasoning', 'rosemary', 'extract', 'sweet', 'baking', 'beaten', 'heavy', 'seeded', 'tin',
                       'uncooked', 'crumb', 'style', 'thin', 'nut', 'coarsely', 'spring', 'cornstarch', 'strip', 
                        'rinsed', 'root', 'quartered', 'head', 'softened', 'container', 
                       'crumbled', 'frying', 'lean', 'cooking', 'roasted', 'warm', 'whipping', 'thawed', 'corn', 'pitted', 
                       'sun', 'kosher', 'bite', 'toasted', 'split', 'melted', 'degree', 'lengthwise', 'romano', 
                       'packed', 'pod', 'rom', 'prepared', 'juiced', 'fluid', 'floret', 'room', 'active', 'seasoned', 
                       'mix', 'deveined', 'lightly', 'anise', 'thai', 'size', 'unsweetened', 'torn', 'wedge', 'sour', 'basmati', 
                       'dark', 'temperature', 'garnish', 'bouillon', 'loaf', 'shell', 'reggiano', 'canola', 
                       'parmigiano', 'round', 'canned', 'crust', 'long', 'broken', 'ketchup', 'bulk', 'cleaned', 
                       'condensed', 'sherry', 'provolone', 'cold', 'spray', 'tamarind', 'pecorino', 
                       'shortening', 'part', 'bottle', 'sodium', 'grain', 'french', 'roast', 'stem', 'link', 'firm', 
    ...
    for i in ingredients:
        ...
        # print("items after alpha", items)

        # Turn everything to lowercase
        items = [word.lower() for word in items]

        # print("items after lower", items)
        ...
        # print("items after unidecode", items)
        
        # Lemmatize words so we can compare words to measuring words
        items = [lemmatizer.lemmatize(word) for word in items]

        # print("items after lemmatizer", items)

        # Gets rid of measuring words/phrases, e.g. heaped teaspoon
        items = [word for word in items if word not in measures]

        # print("items after removing measuring words", items)

        # Get rid of common easy words
        items = [word for word in items if word not in words_to_remove]

        # print("items after words to remove", items)
...
if __name__ == "__main__":
    recipe_df = pd.read_csv("JamieOliver_full.csv")
    recipe_df['ingredients_parsed'] = recipe_df['ingredients'].apply(lambda x: ingredient_parser(x))
    df = recipe_df[['recipe_name', 'ingredients_parsed', 'ingredients','serves', 'cooking_time', 'difficulty', 'nutrition', 'recipe_urls']]
    df = recipe_df.dropna()    
    df.to_csv("parsed_ingredients.csv", index=False)
